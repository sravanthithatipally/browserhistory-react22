...
class App extends Component {
  state = {searchElement: '', historyList: initialHistoryList}

  onChangeInputEle = event => {
    const {historyList, searchElement} = this.state
    this.setState({searchElement: event.target.value})

    const filteredHistory = historyList.filter(each =>
      each.title.toLowerCase().includes(searchElement.toLowerCase()),
    )
  ...
  render() {
    const {searchElement, historyList} = this.state
